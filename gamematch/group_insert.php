...
if(mysqli_num_rows($res) == 0){
	...
	$ret1 = mysqli_query($conn, "insert into groups (group_name, game_id, gr_created_time, gr_introduction) values('$group_name', '$game_id', NOW(), '$gr_introduction')");
	$ret2 = mysqli_query($conn, "insert into belongs (user_id, group_id, role, b_created_time) values('$user_id', LAST_INSERT_ID(), 'manager', NOW())");
}
...
{
	//--added
	mysqli_query($conn, "rollback"); // group 과 belong 에 둘 중 하나라도 insert 를 실패했을 경우, 수행 이전으로 rollback
	//--added
...
{
	//--added
	mysqli_query($conn, "commit"); // group 과 belong 둘 다에 대해 insert 성공했을 경우, commit
	//--added
	
	
    s_msg ('성공적으로 게임그룹이 생성되었습니다');
