...
if($wait['status'] != 'wait'){
	msg("신청서가 대기 상태가 아니므로, 수정이 불가능합니다.");
}
else if($password['password'] != $check_password){
	msg("비밀번호 일치 안함");
}
else{
	$ret1 = mysqli_query($conn, "update applications set description = '$description' where application_id = $application_id");
}
...
{
	//--added
	mysqli_query($conn, "rollback"); //application 수정 실패. 수행 전으로 rollback
	//--added
...
{
	//--added
	mysqli_query($conn, "commit"); // application 수정 성공 commit
	//--added
    s_msg ('성공적으로 가입 신청서 수정이 완료되었습니다.');
